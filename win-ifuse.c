...
#define PACKAGE_NAME "win-ifuse"
#define PACKAGE_URL "https://github.com/uncor3/win-ifuse"
#define PACKAGE_BUGREPORT "https://github.com/uncor3/win-ifuse/issues"
    ...
    if (ret != AFC_E_SUCCESS) {
        ...
        for (i = 0; info[i]; i += 2) {
            if (!strcmp(info[i], "st_size")) {
                ...
                stbuf->st_ctim.tv_sec = (time_t)(atoll(info[i+1]) / 1000000000);
...
static int ifuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler, fuse_off_t offset, struct fuse_file_info *fi)
{
    int i;
    char **dirs = NULL;
    afc_client_t afc = fuse_get_context()->private_data;
...
static int ifuse_create(const char *path, fuse_mode_t mode, struct fuse_file_info *fi)
{
    return ifuse_open(path, fi);
}

static int ifuse_read(const char *path, char *buf, size_t size, fuse_off_t offset, struct fuse_file_info *fi)
{
    uint32_t bytes = 0;
    afc_client_t afc = fuse_get_context()->private_data;
...
static int ifuse_write(const char *path, const char *buf, size_t size, fuse_off_t offset, struct fuse_file_info *fi)
{
    uint32_t bytes = 0;
    afc_client_t afc = fuse_get_context()->private_data;
